...
<div class="main" style="height:60vh;">
    <p class="sign" align="center"><fmt:message key="deposit_request"/></p>
<form class="request" method="post" action="open_deposit">
        <label for="amount">
                 <c:choose>
                                    <c:when test="${errors.amount!=null}">
                                       <fmt:message key="error.amount"/>
                                  </c:when>
                                  <c:when test="${errors.deposit!=null}">
                                                                         <fmt:message key="error.not_enough"/>
                                                                    </c:when>
                                </c:choose>
        </label>
